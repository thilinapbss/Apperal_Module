...
```
TimeoutError: locator.scrollIntoViewIfNeeded: Timeout 29974.328999999998ms exceeded.
Call log:
...
```ts
  ...
  97  | 
  98  |     // Scroll the row into view before clicking
> 99  |     await randomRow.scrollIntoViewIfNeeded();
      |                     ^ TimeoutError: locator.scrollIntoViewIfNeeded: Timeout 29974.328999999998ms exceeded.
  100 |     await this.page.waitForTimeout(200);
  101 | 
  102 |     await randomRow.click();
  103 |     await this.page.waitForLoadState('networkidle');
  104 |     console.log(`Selected customer at random index: ${randomIndex}`);
  105 |   }
  106 | }
  107 | 
```