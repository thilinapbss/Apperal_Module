...
Call log:
  - waiting for locator('//span[text()="WH-CMB"]/ancestor::tr[@role="row"]').first()
    - locator resolved to <tr role="row" tabindex="-1" aria-rowindex="2" aria-selected="true" data-sap-ui-render="" id="__item24-__clone92" data-sap-ui="__item24-__clone92" class="sapMLIB sapMLIB-CTX sapMLIBShowSeparator sapMLIBTypeActive sapMLIBActionable sapMLIBHoverable sapMLIBFocusable sapMListTblRow sapMLIBSelected">…</tr>
...
> 48  |       await warehouseOption.click();
  ...
  51  | 
  52  |       // Fill SemifinishedGood field if provided
  53  |       const semifinishedGood = details[i].SemifinishedGood;
  54  |       if (semifinishedGood) {
  55  |         console.log(`[Row ${i + 1}] Fill Semifinished Good: ${semifinishedGood}`);
  56  |         const semifinishedCell = newRow.locator('td[data-sap-ui-column*="SemifinishedGoods-innerColumn"]');
  57  |         const semifinishedInput = semifinishedCell.locator('input[role="combobox"]');
  58  | 
  59  |         // Type the value directly into the input field
  60  |         await semifinishedInput.click();
  61  |         await this.page.waitForTimeout(200);
  62  |         await semifinishedInput.fill(semifinishedGood);
  63  |         await this.page.waitForTimeout(500);
  64  | 
  65  |         // Press Tab to confirm the entry
  66  |         await this.page.keyboard.press('Tab');
  67  |         await this.page.waitForTimeout(600);
  68  |       }
  69  | 
  70  |       console.log(`✓ Row ${i + 1} completed\n`);
  71  |     }
  72  | 
  73  |     console.log(`✓ All ${details.length} rows filled\n`);
  74  |   }
  75  | 
