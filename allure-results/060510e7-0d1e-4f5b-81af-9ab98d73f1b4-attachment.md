...
```
Error: locator.waitFor: Error: strict mode violation: locator('div[id*="FieldValueHelp::Customer::Dialog::qualifier"]').locator('tbody') resolved to 2 elements:
    1) <tbody>…</tbody> aka locator('tbody').filter({ hasText: 'ValueDescription' })
    2) <tbody></tbody> aka locator('[id="apperal.stylemaster::StyleMasterObjectPage--fe::FormContainer::GeneralInfo::FieldValueHelp::Customer::Dialog::qualifier::::Table-innerTable-table"] tbody')

Call log:
  - waiting for locator('div[id*="FieldValueHelp::Customer::Dialog::qualifier"]').locator('tbody')
