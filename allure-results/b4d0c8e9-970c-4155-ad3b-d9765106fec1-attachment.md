...
> 1671 |     await this.rawMaterialsSection.waitFor({ state: 'visible', timeout: 10000 });
  ...
  1710 |     try {
  1711 |       // Wait for the table body to exist
  1712 |       await this.rawMaterialsTableBody.first().waitFor({ state: 'visible', timeout: 10000 });
  1713 | 
  1714 |       // Wait for any row in the table
  1715 |       const allRows = this.rawMaterialsTableBody.first().locator('tr[role="row"]');
  1716 |       await allRows.first().waitFor({ state: 'visible', timeout: 10000 });
  1717 | 
  1718 |       // Extra wait for row to be fully rendered
  1719 |       await this.page.waitForTimeout(1000);
  1720 |       console.log('  ✓ New row detected in table');
  1721 |     } catch (e) {
  1722 |       console.error(`  ✗ Failed to wait for Raw Materials row: ${e}`);
  1723 |       throw e;
  1724 |     }
  1725 |   }
  1726 | 
  1727 |   async fillRawMaterialRow(itemCode: string, itemName: string) {
  1728 |     try {
  1729 |       console.log(`\n  ╔═════════════════════════════════════════════════════╗`);
  1730 |       console.log(`  ║ Code: ${itemCode}, Name: ${itemName}`);
  1731 |       console.log(`  ╚═════════════════════════════════════════════════════╝`);
  1732 | 
  1733 |       // Get all rows from the table body
  1734 |       const tableBody = this.rawMaterialsTableBody.first();
  1735 |       const allRows = tableBody.locator('tr[role="row"]');
  1736 |       const rowCount = await allRows.count();
  1737 | 
  1738 |       console.log(`    Total rows in table: ${rowCount}`);
  1739 | 
  1740 |       if (rowCount === 0) {
  1741 |         console.error(`    ✗ No rows found in table`);
  1742 |         return false;
  1743 |       }
  1744 | 
  1745 |       // Get the first row (newly created one at top)
  1746 |       const firstRow = allRows.first();
  1747 | 
  1748 |       // Find all input fields in the first row
  1749 |       const inputFields = firstRow.locator('input[type="text"]');
  1750 |       const inputCount = await inputFields.count();
  1751 | 
  1752 |       console.log(`    Input fields in first row: ${inputCount}`);
  1753 | 
  1754 |       if (inputCount < 2) {
  1755 |         console.error(`    ✗ Expected 2 inputs, found ${inputCount}`);
  1756 |         return false;
  1757 |       }
  1758 | 
  1759 |       // Get specific inputs
  1760 |       const itemCodeInput = inputFields.nth(0);
  1761 |       const itemNameInput = inputFields.nth(1);
  1762 | 
  1763 |       // Scroll and wait
  1764 |       await firstRow.scrollIntoViewIfNeeded();
  1765 |       await this.page.waitForTimeout(500);
  1766 | 
  1767 |       // Fill Item Code
  1768 |       console.log(`    → Filling Item Code...`);
  1769 |       await itemCodeInput.click();
  1770 |       await this.page.waitForTimeout(200);
  1771 |       await itemCodeInput.fill(itemCode);
```